...
///
/// Prefers `kokoro-quant-convinteger.onnx`, then falls back to a sorted `.onnx` file.
...
fn init_session(
    ...
    let mut builder = Session::builder()?
        ...
        .map_err(|err| KokoroError::Ort(err.into()))?
...
fn detect_tokens_input(session: &Session) -> Result<String, KokoroError> {
    for input in session.inputs() {
        if input.name() == "input_ids" || input.name() == "tokens" {
            return Ok(input.name().to_string());
        }
    }
    Err(KokoroError::Config(
        "Missing required token input: expected 'input_ids' or 'tokens'".to_string(),
    ))
}

/// Detect whether the speed input expects int32 (true) or float32 (false).
fn detect_speed_type(session: &Session) -> Result<bool, KokoroError> {
    for input in session.inputs() {
        if input.name() == "speed" {
            let type_str = format!("{:?}", input.dtype());
            return Ok(type_str.contains("Int32") || type_str.contains("int32"));
        }
    }
    Err(KokoroError::Config(
        "Missing required 'speed' input in model graph".to_string(),
    ))
}

/// Split phoneme IDs into chunks of at most `MAX_PHONEME_LEN`, preferring punctuation.
fn split_chunks(ids: &[i64], punct_ids: &[i64]) -> Vec<Vec<i64>> {
...
#[cfg(test)]
mod tests {
    use super::{find_onnx_file, split_chunks, MAX_PHONEME_LEN};
    use std::fs;

    #[test]
    fn preferred_onnx_file_wins() {
        let dir = tempfile::tempdir().unwrap();
        fs::write(dir.path().join("z.onnx"), []).unwrap();
        let preferred = dir.path().join("kokoro-quant-convinteger.onnx");
        fs::write(&preferred, []).unwrap();

        assert_eq!(find_onnx_file(dir.path()).unwrap(), preferred);
    }

    #[test]
    fn fallback_onnx_file_is_deterministic() {
        let dir = tempfile::tempdir().unwrap();
        fs::write(dir.path().join("z.onnx"), []).unwrap();
        let first = dir.path().join("a.onnx");
        fs::write(&first, []).unwrap();

        assert_eq!(find_onnx_file(dir.path()).unwrap(), first);
    }

    #[test]
    fn split_chunks_uses_supplied_punctuation_ids() {
        let mut ids: Vec<i64> = (0..MAX_PHONEME_LEN as i64).collect();
        ids[400] = 99;
        ids.extend((0..20).map(|value| value as i64 + 1_000));

        let chunks = split_chunks(&ids, &[99]);

        assert_eq!(chunks[0].last(), Some(&99));
        assert!(chunks.iter().all(|chunk| chunk.len() <= MAX_PHONEME_LEN));
    }
}
