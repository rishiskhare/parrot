        ...
        let ld_library_path = match std::env::var_os("LD_LIBRARY_PATH") {
            Some(existing) => {
                ...
                std::env::join_paths(paths).map_err(|err| {
                    KokoroError::PhonemizerFailed(format!(
                        "failed to build LD_LIBRARY_PATH for espeak-ng: {err}"
                    ))
                })?
            }
...
fn wait_with_timeout(mut child: Child, timeout: Duration) -> Result<Output, KokoroError> {
    let stdout = child.stdout.take();
    let stderr = child.stderr.take();
    let stdout_reader = stdout.map(read_pipe_in_background);
    let stderr_reader = stderr.map(read_pipe_in_background);
    let deadline = Instant::now() + timeout;

    let status = loop {
        if let Some(status) = child.try_wait().map_err(KokoroError::Io)? {
            break status;
        }
        ...
        if Instant::now() >= deadline {
            ...
            return Err(KokoroError::Timeout(timeout));
        }
...
fn read_pipe_in_background<R>(mut pipe: R) -> thread::JoinHandle<std::io::Result<Vec<u8>>>
where
    R: Read + Send + 'static,
{
    thread::spawn(move || {
...
fn join_pipe_reader(
    ...
    reader
        .map(|handle| {
            handle
                .join()
                .unwrap_or_else(|_| Err(std::io::Error::other("failed to join espeak pipe reader")))
        })
        .transpose()
...
mod tests {
    ...
    #[test]
    fn treats_crlf_as_one_boundary() {
        let parts = split_text_parts("One\r\nTwo");
        assert_eq!(
            parts,
            vec![
                TextPart::Text("One".to_string()),
                TextPart::Punct('.'),
                TextPart::Text("Two".to_string()),
            ]
        );
    }
