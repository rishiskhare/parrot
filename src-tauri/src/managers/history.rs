...
impl HistoryManager {
    ...
    pub fn save_transcription(
        ...
        let file_name = format!("parrot-{}.wav", timestamp);
    ...
    pub fn cleanup_old_entries(&self) -> Result<()> {
        ...
        match retention_period {
            crate::settings::HistoryRetentionPeriod::Never => {
                // Don't delete anything
                Ok(())
            }
            crate::settings::HistoryRetentionPeriod::PreserveLimit => {
    ...
    fn cleanup_by_time(
        &self,
        retention_period: crate::settings::HistoryRetentionPeriod,
    ) -> Result<()> {
        let conn = self.get_connection()?;
        ...
        let cutoff_timestamp = match retention_period {
            crate::settings::HistoryRetentionPeriod::Days3 => now - (3 * 24 * 60 * 60), // 3 days in seconds
            crate::settings::HistoryRetentionPeriod::Weeks2 => now - (2 * 7 * 24 * 60 * 60), // 2 weeks in seconds
            crate::settings::HistoryRetentionPeriod::Months3 => now - (3 * 30 * 24 * 60 * 60), // 3 months in seconds (approximate)
