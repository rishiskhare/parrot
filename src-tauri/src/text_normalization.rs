...
impl SpeechTextRenderer {
    ...
    fn push_text(&mut self, text: &str) {
        let had_leading_whitespace = text
            .chars()
            .next()
...
fn normalize_quote_spacing(text: &str) -> String {
    ...
    let mut nearest_left = vec![None; chars.len()];
    let mut nearest_right = vec![None; chars.len()];

    let mut previous = None;
    for (idx, &ch) in chars.iter().enumerate() {
        nearest_left[idx] = previous;
        if !ch.is_whitespace() {
    ...
    for (idx, &ch) in chars.iter().enumerate() {
        ...
        if ch == ' ' {
            let prev_non_space = nearest_left[idx].map(|(_, ch)| ch);
            let next_non_space = nearest_right[idx].map(|(_, ch)| ch);

            if let Some(next_quote) = next_non_space.filter(|c| is_quote_char(*c)) {
                let next_quote_idx = nearest_right[idx].map(|(idx, _)| idx).unwrap_or(idx);
                let after_next_quote = nearest_right
                    .get(next_quote_idx)
                    .and_then(|next| next.map(|(_, ch)| ch));
            ...
            if prev
                ...
                .map(|prev_quote| {
                    let before_prev_quote = idx
                        .checked_sub(1)
                        .and_then(|prev_idx| nearest_left[prev_idx].map(|(_, ch)| ch));
...
fn needs_space_between(prev_left: Option<char>, left: Option<char>, right: Option<char>) -> bool {
    match (prev_left, left, right) {
        ...
        (_, Some(left), Some(right)) if left.is_alphanumeric() && matches!(right, '&' | '\'') => {
            false
        }
...
"#;
    ...
    fn preserves_spaces_around_adjacent_quoted_terms() {
        ...
        let spoken = normalize_text_for_tts(markdown);
        assert!(spoken
            .contains(r#"Navigate to Settings/Integrations: Look for “CSV” or “NPS” settings."#));
