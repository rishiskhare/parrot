...
Thank you for your interest in contributing to Parrot! This guide will help you get started with contributing to this open source text-to-speech application.

## 📖 Philosophy

Parrot aims to be the most forkable text-to-speech app. The goal is to create both a useful tool and a foundation for others to build upon—a well-patterned, simple codebase that serves the community. We prioritize:
...
2. **Clone your fork**:
   ...
   ```bash
   git clone git@github.com:YOUR_USERNAME/parrot.git
   cd parrot
   ```
...
3. **Add upstream remote**:
   ...
   ```bash
   git remote add upstream git@github.com:rishiskhare/parrot.git
   ```
...
Parrot follows a clean architecture pattern:

**Backend (Rust - `src-tauri/src/`):**

- `lib.rs` - Main application entry point with Tauri setup
- `managers/tts.rs` - Streaming TTS synthesis and audio playback
- `managers/model.rs` - Model download, extraction, and lifecycle
- `managers/history.rs` - Utterance storage and retention
...
For more details, see the Architecture section in [README.md](README.md).
...
4. Describe your feature idea including:
   ...
   - How it fits with Parrot's philosophy
...
1. **Search existing issues and PRs** - Check both open AND closed issues and pull requests. Someone may have already addressed this, or there may be a reason it was closed.
   - [Open issues](https://github.com/rishiskhare/parrot/issues)
   - [Closed issues](https://github.com/rishiskhare/parrot/issues?q=is%3Aissue+is%3Aclosed)
   - [Open PRs](https://github.com/rishiskhare/parrot/pulls)
   - [Closed PRs](https://github.com/rishiskhare/parrot/pulls?q=is%3Apr+is%3Aclosed)
...
7. **Create a Pull Request**:
   - Go to the [Parrot repository](https://github.com/rishiskhare/parrot)
...
- Test with different output audio devices
- Try various text inputs and voices
...
By contributing to Parrot, you agree that your contributions will be licensed under the MIT License. See [LICENSE](LICENSE) for details.
